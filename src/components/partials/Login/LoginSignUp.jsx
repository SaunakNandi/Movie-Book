...
const LoginSignUp = () => {
  ...
  const handleClick = () => {
    let errors = {};
    if (showSignUp && name.current.value.length === 0)
      errors.name = "Enter your username";
    if (email.current.value.length === 0) errors.email = "Enter your email Id";
    if (password.current.value.length < 8)
      errors.password = "Password should be of atleast 8 characters long";
  ...
  return (
    <div>
      <button
        className="font-bold w-8 h-9 text-xl text-red-900 cursor-pointer float-right"
        onClick={() => dispatch(handleLoginPopUp(false))}
      >
        X
      </button>
      <form className="text-black p-4" onClick={(e) => e.preventDefault()}>
        <h3 className={showErrorMessage ? errorClass : "hidden"}>
          {showSignUp
            ? "Enter valid email Id and password. Password should be of atleast 8 characters long"
        ...
        {showSignUp && errorMessage.name && (
          <p className={errorClass}>{errorMessage.name}</p>
        )}
        <input
          type="email"
        ...
        />
        {errorMessage.email && (
          <p className={errorClass}>{errorMessage.email}</p>
        )}
        <div className="relative">
          ...
          {errorMessage.password && (
            <p className={errorClass}>{errorMessage.password}</p>
          )}
        </div>
        <div className="">
          <button
        ...
        <p
          className="text-white text-lg text-center cursor-pointer pt-3 hover:text-slate-300"
          onClick={handleSignUp}
        >
          {showSignUp
            ? "Already have an account? login into your existing account"
