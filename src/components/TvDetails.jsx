...
import { fetchTVDetails } from "../store/actions/tv-actions";
...
const TvDetails = () => {
  ...
  return info ? (
    <div
      className="w-full h-[160vh] px-[10%] relative overflow-y-auto"
      style={{
        background: `linear-gradient(rgba(0,0,0,.2),rgba(0,0,0,.3),rgba(0,0,0,.8)),url(https://image.tmdb.org/t/p/original/${info.detail.backdrop_path || info.detail.poster_path})`,
    ...
    >
      ...
      <nav className="h-[10vh] w-full text-zinc-200 flex gap-10 items-center text-2xl">
        ...
        <Link
          onClick={() => navigate(-1)}
          className="mr-2 ri-arrow-left-line hover:text-[#6556CD] font-4xl"
        ></Link>
        ...
        <div className="content ml-[5%] text-white">
          ...
          {info.detail.vote_average && (
            <div className="mt-3 mb-5 flex text-zinc-100 items-center gap-x-5">
              <h2 className="w-fit font-semibold text-2xl leading-4">
                {" "}
                tv rating
              </h2>
          ...
          <h2 className="text-xl font-semibold italic text-zinc-200">
            Created by-{" "}
            {info.detail.created_by
              .map((person) => person.original_name)
              .join(", ")}
          </h2>
          <h1 className="text-2xl font-bold italic text-zinc-200 mt-4">
            {info.detail.tagline}
          </h1>
          ...
          <div className="mt-10">
            ...
            >
              Play Trailer
              <i className="ml-3 text-xl ri-play-large-fill"></i>
            </Link>
          </div>
        ...
        <div className="w-[100%] h[55vh] flex overflow-y-hidden mb-5 p-3">
          {info.detail.seasons.length > 0 ? (
            info.detail.seasons.map((season, i) => (
              <Link
                to={`/tv/details/${id}/season/${season.season_number}`}
                className="w-[12vw] mr-[6%]"
                key={i}
              >
                <img
                  className="h-[40vh] object-center shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)] min-w-[15vw]"
                  src={
                    season.poster_path
            ...
            <h1 className="mb-3 text-4xl text-white font-bol">
