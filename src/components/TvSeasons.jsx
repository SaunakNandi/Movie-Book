...
import { fetchSeasonDetails } from "../store/actions/season-action";
import { Link } from "react-router-dom";
import { removeseason } from "../store/reducers/seasonsSlice";
...
const TvSeasons = () => {
  ...
  useEffect(() => {
    dispatch(fetchSeasonDetails(series, id));
    return () => {
  ...
  return info ? (
    <div
      ...
      style={{
        background: `linear-gradient(rgba(0,0,0,.2),rgba(0,0,0,.3),rgba(0,0,0,.8)),
    ...
    >
      ...
      <div className="w-full flex">
        ...
        <div className="content ml-[5%] text-white">
          ...
          <h1 className="text-2xl font-bold italic text-zinc-200">
            {info.detail.tagline}
          </h1>
        ...
        <div className="w-[100%] h[55vh] flex overflow-y-hidden mb-5 p-3">
          {info.detail.episodes.length > 0 ? (
            info.detail.episodes.map((x, i) => (
              <Link className="w-[12vw] mr-[6%]" key={i}>
                <img
                  className="h-[40vh] object-cover shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)] min-w-[15vw]"
                  src={
                    x.still_path
                      ? `https://image.tmdb.org/t/p/original/${x.still_path}`
                      : no_image
                  }
                  alt=""
                />
